<form id="parsley-form" class="form-horizontal" novalidate name="form_edit">
    <div class="col-md-6 col-sm-12 col-xs-12">
        ...
        <input type="text" class="form-control" name="kode_transaksi" required="required" value="<?php echo $record->kode_transaksi ?>">
    </div>
    <div class="col-md-6 col-sm-12 col-xs-12">
        <label class="control-label">Nama Transaksi<span class="required">*</span></label>
    ...
    <div class="col-md-6 col-sm-12 col-xs-12">
        ...
        <input type="text" class="form-control" name="harga" required="required" value="<?php echo $record->harga ?>">
    </div>
    <div class="col-md-6 col-sm-12 col-xs-12">
        <label class="control-label">Stok<span class="required">*</span></label>
        <input type="text" class="form-control" name="stok" required="required" value="<?php echo $record->stok ?>">
    </div>
...
<script>
    $("#parsley-form").parsley().on('field:validated',function(){}).on('form:submit', function(){
        var link = "<?php echo base_url('app/update_transaksi/'.$record->id) ?>",
    ...
    $(".cancel").on('click', function() {
        if(confirm('<?php echo lang('dialog_abandon_changes') ?>')) {
            // Load Form Add
            let form_add = sendAjax('<?php echo base_url('app/create_transaksi') ?>');
